...
const themes: Record<string, Theme> = {
  dedication: {
    ...
    latinName: 'Praefatio et index',
    image: '/assets/dedication-pliny-vespasian.jpg',
    alt: 'Pliny dedicating his Natural History in an imperial Roman library',
    motto: 'The whole world in thirty-seven books',
    latinMotto: 'Mundus universus libris XXXVII',
  },
  cosmos: {
    name: 'Cosmos & Elements',
    latinName: 'Mundus et elementa',
    image: '/assets/cosmos-four-elements.jpg',
    alt: 'An ancient cosmological plate of the heavens and four elements',
    motto: 'Nature is nowhere greater than in the whole',
    latinMotto: 'Natura nulla parte maior quam tota',
  },
  geography: {
    name: 'Lands & Peoples',
    latinName: 'Terrae et gentes',
    image: '/assets/geography-mediterranean.jpg',
    alt: 'An illuminated map of the Mediterranean with cities, rivers, and mountains',
    motto: 'Every shore enters the record',
    latinMotto: 'Omne litus in memoriam venit',
  },
  humanity: {
    name: 'Humanity & Healing',
    latinName: 'Homo et medicina',
    image: '/assets/humanity-anatomy-healing.jpg',
    alt: 'A Roman anatomical and healing plate with figures and medicinal instruments',
    motto: 'The human creature, fragile and ingenious',
    latinMotto: 'Homo fragilis atque ingeniosus',
  },
  terrestrial: {
    name: 'Terrestrial Animals',
    latinName: 'Animalia terrestria',
    image: '/assets/terrestrial-animals.jpg',
    alt: 'A natural history plate of elephants, lions, deer, and other land animals',
    motto: 'Nature speaks in signs',
    latinMotto: 'Natura signis loquitur',
  },
  marine: {
    name: 'Waters & Marine Life',
    latinName: 'Aquae et animalia marina',
    image: '/assets/marine-waters-remedies.jpg',
    alt: 'An ancient marine plate of fish, shells, sea creatures, springs, and remedies',
    motto: 'The sea keeps its own medicine',
    latinMotto: 'Mare medicinam suam servat',
  },
  flight: {
    name: 'Birds & Insects',
    latinName: 'Aves et insecta',
    image: '/assets/birds-insects-flight.jpg',
    alt: 'An illuminated plate of birds, bees, butterflies, and winged creatures',
    motto: 'The smallest wings bear mysteries',
    latinMotto: 'Minimae alae mysteria ferunt',
  },
  botany: {
    name: 'Trees, Crops & Remedies',
    latinName: 'Arbores fruges remedia',
    image: '/assets/botany-agriculture-remedies.jpg',
    alt: 'A botanical plate of ancient trees, crops, flowers, and medicinal herbs',
    motto: 'The forest is a gift to humankind',
    latinMotto: 'Silvae munus homini',
  },
  minerals: {
    name: 'Metals, Stones & Art',
    latinName: 'Metalla lapides artes',
    image: '/assets/minerals-metals-arts.jpg',
    alt: 'An ancient mineralogical plate of ores, gems, pigments, sculpture, and metalwork',
...
export default function Home() {
  ...
  const [plateViewerIndex, setPlateViewerIndex] = useState<number | null>(null);
  const [plateViewerMode, setPlateViewerMode] = useState<'study' | 'atlas'>('study');
    ...
    abortable(loadBook(meta), controller.signal)
      .then(async (book) => {
        ...
        const targetIllustration = chapterIllustration({
          ...
          chapterLatinTitle: targetChapter.latinTitle,
          englishSubheadings: targetChapter.englishChapters,
  ...
  useEffect(() => {
    if (plateViewerIndex === null) return;
    const dialog = plateViewerDialogRef.current;
    if (!dialog) return;
    const previousOverflow = document.body.style.overflow;
    const finishClose = () => {
      setPlateViewerIndex(null);
      setPlateViewerMode('study');
  ...
  const illustration = chapterIllustration({
    ...
    ordinal: activeLocation.ordinal,
  });
  const [mainIllustrationPanel, ...detailIllustrationPanels] = illustration.panels;
  const illustrationStyle = illustration.style as Record<string, string>;
  const viewerPanel = plateViewerIndex === null ? null : illustration.panels[plateViewerIndex] ?? null;
  const viewerField = viewerPanel?.field ?? 'main';
  const framedViewerSize = viewerPanel
    ? viewerPanel.cell
      ? 292
      : viewerPanel.source.viewerKind === 'chapter-scene'
        ? 100
      : Math.max(112, Number.parseFloat(illustrationStyle[`--plate-${viewerField}-size`]) * 0.76)
    : 100;
  const plateViewerStyle = viewerPanel ? {
    '--viewer-image': illustrationStyle[`--plate-${viewerField}-image`],
    '--viewer-image-set': illustrationStyle[`--plate-${viewerField}-image-set`],
    '--viewer-x': illustrationStyle[`--plate-${viewerField}-x`],
    '--viewer-y': illustrationStyle[`--plate-${viewerField}-y`],
    '--viewer-size': `${framedViewerSize}%`,
    '--viewer-focus-x': `${viewerPanel.focus[0]}%`,
    '--viewer-focus-y': `${viewerPanel.focus[1]}%`,
  } as CSSProperties : undefined;
  const plateViewerDescription = viewerPanel
    ? plateViewerMode === 'study'
      ? viewerPanel.source.viewerKind === 'cell'
        ? 'Modern editorial illustration. A complete, uncropped study from this receipt-backed six-scene atlas.'
        : viewerPanel.source.viewerKind === 'chapter-scene'
          ? 'Modern editorial illustration. A complete, independently generated scene assigned only to this chapter and shown uncropped.'
          : 'Modern editorial illustration. An enlarged crop from a larger atlas; open the full source to inspect its complete context.'
      : `Modern editorial source atlas with the selected study marked. ${viewerPanel.source.description}.`
    : '';

  const openPlateViewer = (index: number, trigger: HTMLButtonElement) => {
    plateViewerTriggerRef.current = trigger;
    setPlateViewerMode('study');
    setPlateViewerIndex(index);
  };

  const movePlateViewer = (direction: -1 | 1) => {
    if (plateViewerIndex === null || illustration.panels.length < 2) return;
    setPlateViewerMode('study');
    setPlateViewerIndex((plateViewerIndex + direction + illustration.panels.length) % illustration.panels.length);
  };
  ...
  return (
    ...
    >
        ...
        <article className="book-spread">
          ...
          <section id="illustrated-plate-page" ref={platePageRef} className={`page page-right${mobilePlateOpen ? ' mobile-plate-is-open' : ''}`}>
            ...
            <figure
              ...
              className={`illustration-plate layout-${illustration.layout} mark-${illustration.mark}`}
              data-family={illustration.family}
              data-match-source={illustration.matchSource}
              data-subject={illustration.subject}
              style={illustrationStyle as CSSProperties}
              aria-labelledby="illustrated-plate-caption"
            >
              <span className="plate-cabinet">
                <button
                  className="plate-field plate-field-main"
                  type="button"
                  onClick={(event) => openPlateViewer(0, event.currentTarget)}
                  aria-label={`Enlarge ${mainIllustrationPanel.accessibleLabel}`}
                ><span className="plate-expand-mark" aria-hidden="true">↗</span></button>
                {detailIllustrationPanels.length > 0 && (
                  <span className="plate-detail-column">
                    {detailIllustrationPanels.map((panel, index) => (
                      <button
                        className={`plate-field plate-field-detail plate-field-${panel.field}`}
                        type="button"
                        onClick={(event) => openPlateViewer(index + 1, event.currentTarget)}
                        aria-label={`Enlarge ${panel.accessibleLabel}`}
                        key={panel.field}
                      ><span className="plate-expand-mark" aria-hidden="true">↗</span></button>
                    ))}
                  </span>
              ...
              <details className="folio-apparatus" key={`notes:${illustration.instanceKey}`}>
                ...
                <div className="folio-apparatus-body">
                  ...
                  <p className="editorial-disclosure">
                    Modern editorial plate · {illustration.matchSource === 'campaign'
                      ? 'mapped from the chapter taxonomy'
                      : illustration.matchSource === 'certified-atlas-cell'
                      ? 'certified source study assigned one-to-one'
                      : illustration.matchSource === 'chapter-scene'
                      ? 'independently illustrated for this chapter'
                      : illustration.matchSource === 'curated'
                      ? 'hand-matched to this chapter'
                      : illustration.matchSource === 'title'
                        ? 'matched from the chapter heading'
                        : illustration.matchSource === 'subheading'
                          ? 'matched from Bostock–Riley subheadings'
                          : 'book-family fallback'} · Latin derives from the pinned Mayhoff/Perseus transcription; interventions are disclosed
                  </p>
                  <dl>
      ...
      {viewerPanel && (
        ...
        >
          <section className="plate-viewer-shell" style={plateViewerStyle}>
            <header className="plate-viewer-header">
              <div>
                <p>PLATE</p>
                <h2 id="plate-viewer-title">{viewerPanel.cellLabel ?? illustration.englishCaption}</h2>
              </div>
            ...
            <div className={`plate-viewer-media is-${plateViewerMode}`}>
              {plateViewerMode === 'study' && ['cell', 'chapter-scene'].includes(viewerPanel.source.viewerKind) && viewerPanel.source.viewerImage ? (
                // Receipt-backed cell and chapter-scene URLs are already bounded responsive pipeline artifacts.
                <picture className="plate-viewer-picture">
                  {viewerPanel.source.viewerPreferredImage && <source srcSet={viewerPanel.source.viewerPreferredImage} type="image/avif" />}
                  <img src={viewerPanel.source.viewerImage} alt={viewerPanel.accessibleLabel} />
                </picture>
              ) : plateViewerMode === 'study' ? (
                <div className="plate-viewer-crop" role="img" aria-label={`${viewerPanel.accessibleLabel} Enlarged framed study from the source atlas.`} />
              ) : (
                <div className="plate-viewer-atlas">
                  {/* Generated atlas derivatives are source-pinned and intentionally bypass runtime image transformation. */}
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={viewerPanel.source.desktopImage} alt={`Complete modern editorial source plate: ${viewerPanel.source.description}.`} />
                  <span className="plate-viewer-focus" aria-hidden="true"><i /></span>
                </div>
              )}
            </div>

            <footer className="plate-viewer-footer">
              <div className="plate-viewer-pagination" aria-label="Illustration studies">
                <button type="button" onClick={() => movePlateViewer(-1)} disabled={illustration.panels.length < 2} aria-label="Previous illustration study">←</button>
                {illustration.panels.map((panel, index) => (
                  <button
                    type="button"
                    className={index === plateViewerIndex ? 'is-current' : ''}
                    aria-pressed={index === plateViewerIndex}
                    onClick={() => { setPlateViewerMode('study'); setPlateViewerIndex(index); }}
                    key={panel.field}
                  >{panel.label}</button>
                ))}
                <button type="button" onClick={() => movePlateViewer(1)} disabled={illustration.panels.length < 2} aria-label="Next illustration study">→</button>
              </div>
              <details className="plate-viewer-about" key={`about:${plateViewerIndex}:${plateViewerMode}`}>
                <summary>About this image</summary>
                <p>{plateViewerDescription} <a href="/edition#rights">Image provenance</a>.</p>
              </details>
              {viewerPanel.source.viewerKind !== 'chapter-scene' && (
                <div className="plate-viewer-modes" role="group" aria-label="Illustration view">
                  <button type="button" className={plateViewerMode === 'study' ? 'is-current' : ''} aria-pressed={plateViewerMode === 'study'} onClick={() => setPlateViewerMode('study')}>Study</button>
                  <button type="button" className={plateViewerMode === 'atlas' ? 'is-current' : ''} aria-pressed={plateViewerMode === 'atlas'} onClick={() => setPlateViewerMode('atlas')}>Full source plate</button>
                </div>
              )}
            </footer>
          </section>
