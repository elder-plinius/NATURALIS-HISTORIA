...
export const NORMALIZED_TIME = new Date('2026-08-24T00:00:00.000Z');
export const ARCHIVE_PREFIX = 'naturalis-historia';

const chapterScenePrefix = 'assets-source/chapter-scenes-v1/';
const excludedPrefixes = [
  '.git/', '.next/', '.openai/', '.vinext/', '.wrangler/', 'assets-source/archive/',
...
const excludedSuffixes = [
  ...
  '.pfx', '.sqlite', '.sqlite3', '.tar', '.tgz', '.wav', '.zip',
];
...
export const includeReleasePath = (relative, profile = RELEASE_PROFILES.COMPREHENSIVE_SOURCE) => {
  ...
  if (excludedExact.has(relative) || excludedPrefixes.some((prefix) => relative.startsWith(prefix))) return false;
...
export const enumerateSourceAssetPaths = (root) => {
  ...
  if (!selected.length) throw new Error('No chapter-scene source assets were found.');
  return selected;
};
