...
assert.match(css, /\.plate-field::before\s*\{[\s\S]*?background-size:\s*cover;/);
assert.doesNotMatch(css, /\.layout-(?:hero|map|orbit)[^}]*background-size:\s*cover/);
...
assert.match(page, /illustration\.matchSource === 'campaign'[\s\S]*mapped from the chapter taxonomy[\s\S]*illustration\.matchSource === 'curated'[\s\S]*hand-matched to this chapter/);
...
assert.match(page, /illustration\.panels\.map[\s\S]*aria-pressed=\{index === plateViewerIndex\}/);
assert.match(page, /viewerPanel\.source\.viewerKind === 'cell'[\s\S]*complete, uncropped study/);
assert.match(page, /Modern editorial source atlas with the selected study marked/);
assert.match(page, /<p>PLATE<\/p>[\s\S]*<details className="plate-viewer-about"[\s\S]*<summary>About this image<\/summary>/);
...
assert.match(css, /\.plate-viewer-atlas img[\s\S]*object-fit:\s*contain/);
assert.match(css, /\.layout-map \.plate-detail-column[^}]*pointer-events:\s*auto/);
assert.match(css, /\.layout-orbit \.plate-detail-column[^}]*pointer-events:\s*auto/);
assert.match(css, /@media \(max-width: 620px\)[\s\S]*\.plate-viewer-footer \{ grid-template-columns: minmax\(0,1fr\)/);
...
console.log('Verified seeded bounded living-ink diffusion, stable scroll settlement, cover-safe crops, responsive plate delivery, strict corpus framing, mode-correct bilingual accessibility, history/share wiring, reduced motion, cautious book prefetch, security headers, and bounded fetch behavior');
