...
const artworkManifest = JSON.parse(await readFile(path.join(root, 'assets-source', 'chapter-artwork-manifest.json'), 'utf8'));
const fail = (condition, message) => { if (!condition) throw new Error(message); };
const canonicalChapterSceneCount = 1_065;
fail(manifest.totalChapters === canonicalChapterSceneCount, `Expected the canonical ${canonicalChapterSceneCount} chapters, found ${manifest.totalChapters}`);
const chapterAssignments = Object.entries(artworkManifest.assignments ?? {});
fail(chapterAssignments.length === canonicalChapterSceneCount, `Expected ${canonicalChapterSceneCount} chapter-scene assignments, found ${chapterAssignments.length}`);
fail(chapterAssignments.every(([, assignment]) => assignment.kind === 'chapter-scene'), 'Every chapter route must use a standalone chapter scene');
let count = 0;
let bytes = 0;
...
const replacementScene = await readFile(path.join(publicRoot, 'read', '34', '13.html'), 'utf8');
fail(
  replacementScene.includes('leaf-plate-contained')
    && replacementScene.includes('/assets/chapter-b34-c013.')
    && replacementScene.includes('.w1536.avif')
    && !replacementScene.includes('-atlas-cell-'),
  'Former multi-panel route is not delivered as a complete standalone scene',
);
