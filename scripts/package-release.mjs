...
const packageSourceArchive = async (options, output) => {
  ...
  const paths = enumerateGitPaths(profile);
  ...
  if (profile === RELEASE_PROFILES.PUBLIC_REPO) {
    const excludedSourceAssetPaths = enumerateSourceAssetPaths(root);
  ...
  try {
    await withTemporaryDirectory('naturalis-historia-package-', async (temporary) => {
      ...
      writeNormalizedJson(profilePath, {
        archiveCeilingBytes,
        excludedChapterSceneClasses: profile === RELEASE_PROFILES.PUBLIC_REPO
          ? ['accepted-master', 'reject-evidence']
          : [],
        ...
        schemaVersion: 1,
        sourceAssetManifest: profile === RELEASE_PROFILES.PUBLIC_REPO
      ...
      await validateFrozenSnapshot(snapshots, digests, () => enumerateGitPaths(profile), `${profile} source`);
      const partialChecksum = writePartialText(
        checksumOutput,
        `${archiveDigest}  ${path.basename(output)}\n`,
        partials,
      );
      validateFrozenMetadata(snapshots, () => enumerateGitPaths(profile), `${profile} source`);
      if (profile === RELEASE_PROFILES.PUBLIC_REPO) {
        validateFrozenMetadata(excludedSourceAssetSnapshots, () => enumerateSourceAssetPaths(root), 'public-repo excluded source-asset');
      }
      commitArtifacts([
