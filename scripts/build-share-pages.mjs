...
for (let index = 0; index < records.length; index += 1) {
  ...
  const panelSource = illustration.panels?.[0]?.source;
  const containedSource = panelSource && (
    panelSource.viewerKind === 'chapter-scene'
    || illustration.matchSource === 'certified-atlas-cell'
  )
    ? {
        fallback: panelSource.viewerImage,
        avif: panelSource.viewerPreferredImage,
        width: panelSource.viewerKind === 'chapter-scene' ? 1536 : 512,
        height: panelSource.viewerKind === 'chapter-scene' ? 1024 : 512,
      }
    : null;
  const image = containedSource ? null : IMAGE_SOURCES[illustration.images[0]];
  if (!containedSource && !image) {
    throw new Error(`Missing share-page image source for ${book.number}:${chapter.id}`);
  }
  const shareImage = containedSource ?? {
    fallback: image.mobile.fallback,
    mobileAvif: image.mobile.imageSet.match(/url\("([^"]+\.avif)"\)/)?.[1],
    avif: image.desktop.imageSet.match(/url\("([^"]+\.avif)"\)/)?.[1],
    width: image.width,
    height: image.height,
  };
  const shareSrcset = containedSource
    ? `${shareImage.avif} ${shareImage.width}w`
    : `${shareImage.mobileAvif} 1024w, ${shareImage.avif} ${shareImage.width}w`;
  const leafStyle = containedSource
    ? '--leaf-size:100%;--leaf-left:0%;--leaf-top:0%;--leaf-x:50%;--leaf-y:50%'
    : `--leaf-size:${illustration.style['--plate-main-size']};--leaf-left:${illustration.style['--plate-main-x-offset']};--leaf-top:${illustration.style['--plate-main-y-offset']};--leaf-x:${illustration.style['--plate-main-x']};--leaf-y:${illustration.style['--plate-main-y']}`;
...
<body>
  ...
  <main>
    <article class="reading-leaf">
      ...
      <figure>
        <div class="leaf-plate-crop${containedSource ? ' leaf-plate-contained' : ''}"><a class="leaf-plate-trigger" href="${shareImage.avif}" target="_blank" rel="noopener" aria-haspopup="dialog" aria-controls="leaf-plate-dialog" aria-label="Open the complete illustration"><picture style="${leafStyle}"><source type="image/avif" srcset="${shareSrcset}" sizes="(max-width: 760px) 100vw, 920px"><img src="${shareImage.fallback}" width="${shareImage.width}" height="${shareImage.height}" alt="${escapeHtml(illustration.alt)}"></picture><span class="leaf-expand" aria-hidden="true">Expand image</span></a></div>
