...
const chapterSceneAudit = chapterSceneAuditSourceFor(2, chapter.id);
assert.ok(chapterScene, `Generated chapter-scene source is missing for ${chapterKey}`);
assert.ok(chapterSceneAudit, `Generated chapter-scene audit source is missing for ${chapterKey}`);
assert.equal(chapterSceneAudit.sourceArtifact, assignment.sourceArtifact);
assert.equal(chapterSceneAudit.sourceSha256, assignment.sourceSha256);
assert.equal(chapterScene.logicalPath, chapterSceneAudit.logicalPath);
assert.deepEqual(chapterScene.desktop, chapterSceneAudit.desktop);
assert.deepEqual(chapterScene.mobile, chapterSceneAudit.mobile);
assert.deepEqual(chapterScene.derivatives, chapterSceneAudit.derivatives);
...
assert.equal(chapterSceneAsset.pipelineRevision, chapterSceneAudit.pipelineRevision);
assert.deepEqual(chapterSceneAsset.derivatives, chapterSceneAudit.derivatives);
assert.equal(new Set(chapterSceneAudit.derivatives).size, 4);
...
if (chapterSceneSourceMode === 'masters') {
  ...
  const expectedNameHash = createHash('sha256')
    .update(`${assignment.sourceSha256}:${chapterSceneAudit.pipelineRevision}`)
    .digest('hex')
    .slice(0, 8);
  assert.deepEqual(
    chapterSceneAudit.derivatives.map((derivative) => path.basename(derivative)).sort(),
    [
...
for (const derivative of chapterSceneAudit.derivatives) {
  await access(path.join(root, 'public', derivative.replace(/^\//, '')));
}
