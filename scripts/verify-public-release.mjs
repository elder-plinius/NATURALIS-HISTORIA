...
const forbiddenPhrases = ['owner-only delivery', 'Private edition'];
const privateSitesHostname = /\b(?:[a-z0-9-]+\.)+chatgpt\.site\b/iu;
...
while (stack.length) {
  ...
  for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
    ...
    if (privateSitesHostname.test(text)) {
      throw new Error(`Private Sites hostname remains in ${path.relative(root, absolute)}`);
    }
