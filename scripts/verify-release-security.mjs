...
for (const pattern of ['.env*', '*.key', '*.p12', '*.pfx', '*.der', '*.sqlite', '*.db', '.npmrc', '.netrc', '/dist/', '/.wrangler/', '/.openai/', '/assets-source/archive/', '/node_modules']) {
...
console.log('Verified pinned install metadata, gated deploy scripts, Cloudflare config, CSP/header parity, CI controls, packaging exclusions, and zero credential/private-path signatures in public text files.');
